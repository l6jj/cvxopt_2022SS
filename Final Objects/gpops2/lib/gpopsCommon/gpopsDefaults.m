...
if isfield(setup, 'nlp')
    ...
    if isfield(setup.nlp, 'solver')
        ...
        if isequal(setup.nlp.solver,'snopt'),
            if isempty(which('snoptcmex')),
               error('NLP Solver SNOPT Not Found');
            end
        end
        if isequal(setup.nlp.solver,'ipopt'),
            if isempty(which('ipopt')),
               error('NLP Solver IPOPT Not Found');
            end
        end
    else
        % default nlp solver = 'snopt'
        disp('    Using Default NLP Solver IPOPT');
...
if isfield(setup, 'derivatives');
    ...
    if isfield(setup.derivatives, 'derivativelevel');
        ...
        disp('    Using Default Derivative Level: Second');
