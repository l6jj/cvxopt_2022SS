...
theta1=input.phase(1).state(:,2);
X1    =input.phase(1).state(:,3);
Y1    =input.phase(1).state(:,4);
m1    =input.phase(1).state(:,5);
P1    =input.phase(1).control(:,1);
alpha1=input.phase(1).control(:,2);
...
rho0=auxdata.rho0;
rho = rho0*exp(-Y1./H);
dynamic_pressure1 = 0.5*rho.*v1.^2;

CD1=0.2;%chazhi_x(alpha1.*180/pi,v1./320.7);
CL1=0.3;%chazhi_cl(alpha1.*180/pi,v1./320.7);
D1= dynamic_pressure1.*S.*CD1;
L1 = dynamic_pressure1.*S.*CL1;
...
vdot1=-(P1.*cosa1+D1)./m1-g.*sin1;
thetadot1=(-P1.*sina1+L1)./(m1.*v1)-g.*cos1./v1;
Xdot1=v1.*cos1;
Ydot1=v1.*sin1;
mdot1=-P1./(g.*Isp);

phaseout(1).dynamics = [vdot1 thetadot1 Xdot1 Ydot1 mdot1];

%---------------------%
% Dynamics in Phase 2 %
%---------------------%
v2    =input.phase(2).state(:,1);
theta2=input.phase(2).state(:,2);
X2    =input.phase(2).state(:,3);
Y2    =input.phase(2).state(:,4);
m2    =input.phase(2).state(:,5);
P2    =input.phase(2).control(:,1);
alpha2=input.phase(2).control(:,2);

rho2 = rho0*exp(-Y2./H);
dynamic_pressure2 = 0.5*rho2.*v2.^2;

CD2=0.2;%chazhi_x(alpha2.*180/pi,v2./320.7);
CL2=0.3;%chazhi_cl(alpha2.*180/pi,v2./320.7);
D2 = dynamic_pressure2.*S.*CD2;
L2 = dynamic_pressure2.*S.*CL2;
...
vdot2=-(P2.*cosa2+D2)./m2-g.*sin2;
thetadot2=(-P2.*sina2+L2)./(m2.*v2)-g.*cos2./v2;
Xdot2=v2.*cos2;
Ydot2=v2.*sin2;
mdot2=-P2./(g.*Isp);

phaseout(2).dynamics = [vdot2 thetadot2 Xdot2 Ydot2 mdot2];
